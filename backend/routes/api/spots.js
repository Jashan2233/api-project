...
// Add an Image to a spoit with SpotId -- fixed!
...
router.post(
  ...
  async (req, res, next) => {
    ...
    if (spot) {
      ...
      const imageData = await SpotImage.scope("defaultScope").findByPk(
        newImageId
      );
      res.json(imageData);
    } else {
...
router.post(
  ...
  async (req, res, next) => {
    ...
    for (let booking of bookings) {
      ...
      if (
        !(
          endDateValue.getTime() <= startValue.getTime() ||
          startDateValue.getTime() >= endValue.getTime()
        )
      ) {
        return res.status(403).json({
